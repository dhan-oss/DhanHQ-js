...
export class DhanFeed {
  ...
  private createSubscriptionPacket(
    ...
    const actualFeedRequestCode = status
      ? this.subscriptionCode
      : this.subscriptionCode + 1;

    const header = this.createHeader(
      actualFeedRequestCode,
      83 + 4 + numInstruments * 21,
      this.clientId
    );
    ...
    const subscriptionPacket = Buffer.concat([
      header,
      numInstrumentsBytes,
      instrumentInfo,
    ]);
  ...
  async unsubscribe(feedRequestCode: number, symbols: any) {
    this.instruments = this.instruments.filter(
      (instrument) =>
        !symbols.some(
          (symbol) => symbol[0] === instrument[0] && symbol[1] === instrument[1]
        )
    );
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      console.log("Unsubscribing from:", symbols);
      const unsubscribePacket = this.createUnsubscribePacket(
        feedRequestCode,
        symbols
      );
  ...
  private processMarketStatusPacket(data: Buffer): MarketStatusResponse {
    if (data.length >= 1) {
      ...
      const marketStatusResponse: MarketStatusResponse =
        new MarketStatusResponse("Market Status Packet", responseCode, status);
  ...
  private processMarketDepthPacket(data: Buffer): MarketDepthResponse {
    ...
    if (data.length < headerLength + maxDepthLevels * depthLevelSize) {
      console.warn(
        `Market Depth packet data is shorter than expected. Expected at least ${
          headerLength + maxDepthLevels * depthLevelSize
        } bytes, received ${data.length} bytes.`
      );
    }
    return result;
  }
